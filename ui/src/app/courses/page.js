  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        // Assuming Admin Panel is running on port 3000
    ...
    <div className="w-full flex flex-col items-center justify-start min-h-screen bg-slate-50 dark:bg-slate-900 font-sans transition-colors duration-300 relative overflow-hidden">
      ...
      <section className="relative w-full pt-32 pb-20 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-slate-900 dark:text-white mb-6 tracking-tight animate-fade-in-up delay-100">
          ...
          <div className="max-w-md mx-auto relative animate-fade-in-up delay-300">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shadow-lg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all"
            />
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {activeCourses.length > 0 ? (
            activeCourses.map((course) => (
              <div
                key={course.id}
                className="group relative bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-3xl p-6 border border-white/50 dark:border-white/10 shadow-sm hover:shadow-xl hover:bg-white dark:hover:bg-slate-800 hover:-translate-y-2 transition-all duration-500 cursor-pointer flex flex-col h-full"
                onClick={() => handleCourseClick(course)}
              >
                {/* Decoration */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-purple-100 to-transparent dark:from-purple-900/30 rounded-tr-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                <div className="flex-1">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-50 to-amber-50 dark:from-slate-700 dark:to-slate-600 border border-white dark:border-slate-500 shadow-inner flex items-center justify-center text-4xl mb-6 group-hover:scale-110 transition-transform duration-500">
                    {course.icon}
                  </div>
                ...
                <div className="mt-auto space-y-4 pt-6 border-t border-slate-100 dark:border-slate-700">
                  ...
                  <button className="w-full py-3 rounded-xl bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 font-bold text-sm group-hover:bg-purple-600 group-hover:text-white transition-all duration-300 flex items-center justify-center gap-2">
      ...
      {showPopup && selectedCourse && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all duration-300">
          <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl w-full max-w-3xl rounded-3xl shadow-2xl border border-white/20 dark:border-white/10 overflow-hidden animate-zoom-in relative">
            ...
            <div className="px-8 pb-8 -mt-16 relative">
              <div className="w-24 h-24 rounded-3xl bg-white dark:bg-slate-800 shadow-xl flex items-center justify-center text-5xl border-4 border-white dark:border-slate-700 mb-6">
                {selectedCourse.icon}
              </div>

              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                  <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{selectedCourse.title}</h2>
                  <div className="flex gap-3 mt-2">
                    <span className="px-3 py-1 rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 text-xs font-bold uppercase tracking-wider">
                      {selectedCourse.duration}
                    </span>
                    <span className="px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 text-xs font-bold uppercase tracking-wider">
                      Premium
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wide">Course Fee</p>
                  <p className="text-3xl font-bold text-slate-900 dark:text-white">{selectedCourse.price}</p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-700 mb-8">
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={() => setShowPopup(false)}
                  className="px-8 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-all duration-300"
                >
                  Close
                </button>
              </div>
      ...
      <ScrollAnimation className="w-full max-w-4xl mx-auto px-6 mb-24">
        <h2 className="text-3xl font-bold text-slate-900 dark:text-white text-center mb-12">Common Questions</h2>
        <div className="grid gap-4">
          <AnimatedTile
            ...
            className="text-left bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700"
          />
          <AnimatedTile
