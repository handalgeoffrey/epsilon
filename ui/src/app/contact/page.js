...
export default function Contact() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center pt-navbar w-full bg-[#f8fafc]">
      {/* Hero Banner */}
      <div className="w-full flex flex-col items-center justify-center py-12 mb-8 relative overflow-hidden">
        ...
        <p className="text-lg text-gray-700 text-center mt-4 z-10">Get in touch for any queries about our courses or admissions. We’re here to help you succeed!</p>
      </div>
      {/* Main Card */}
      <div className="w-full max-w-5xl mx-auto glass shadow-2xl rounded-3xl px-0 py-0 flex flex-col md:flex-row items-stretch overflow-hidden motion-fade-in relative">
        ...
        {/* Contact Info Column */}
        <div className="flex-1 flex flex-col gap-6 px-8 py-12 md:py-16 justify-center bg-white/40 backdrop-blur-lg">
        ...
        {/* Contact Form Column */}
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-6 px-8 py-12 md:py-16 justify-center bg-white/60 backdrop-blur-lg">
          <h2 className="text-2xl font-bold gradient-text mb-2 text-center">Send Us a Message</h2>
          <div className="flex flex-col gap-2">
            <label className="font-semibold flex items-center gap-2"><FaUser className="text-brandPurple" /> Name</label>
            <input name="name" value={form.name} onChange={handleChange} className="bg-white/60 backdrop-blur-lg px-4 py-3 rounded-xl border border-white/40 shadow focus:border-brandPurple focus:ring-2 focus:ring-brandPurple/30 transition-all" required placeholder="Enter your name" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="font-semibold flex items-center gap-2"><FaEnvelope className="text-brandPurple" /> Email</label>
            <input name="email" type="email" value={form.email} onChange={handleChange} className="bg-white/60 backdrop-blur-lg px-4 py-3 rounded-xl border border-white/40 shadow focus:border-brandPurple focus:ring-2 focus:ring-brandPurple/30 transition-all" required placeholder="Enter your email" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="font-semibold flex items-center gap-2"><FaPhone className="text-brandPurple" /> Phone</label>
            <input name="phone" value={form.phone} onChange={handleChange} className="bg-white/60 backdrop-blur-lg px-4 py-3 rounded-xl border border-white/40 shadow focus:border-brandPurple focus:ring-2 focus:ring-brandPurple/30 transition-all" required placeholder="Enter your phone number" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="font-semibold flex items-center gap-2"><FaCommentDots className="text-brandPurple" /> Message</label>
            <textarea name="message" value={form.message} onChange={handleChange} className="bg-white/60 backdrop-blur-lg px-4 py-3 rounded-xl border border-white/40 shadow focus:border-brandPurple focus:ring-2 focus:ring-brandPurple/30 transition-all min-h-[120px] resize-none" required placeholder="Type your message here..." />
          </div>
